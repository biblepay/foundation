...
namespace NBitcoin.Crypto
{
    ...
    public abstract class BBPSmallCore : DigestEngine
    {
    ...
    0xd18d5c777cb934d1L, 0xf9e1180818e908f9L,
    ...
    0x6d0c61d661dad66dL, 0x2c624e3a4e583a2cL
        ...
        protected Digest copyState(BBPSmallCore dst)
        {
            Array.Copy(H, 0, dst.H, 0, (int)(uint)H.Length);
        ...
        protected override void doPadding(byte[] output, uint outputOffset)
        {
            ...
            processBlock(buf);
            Array.Copy(H, 0, G, 0, (int)(uint)H.Length);
            doPermP(G);
            for (uint i = 0; i < 4; i++)
                encodeBELong(H[i + 4] ^ G[i + 4], buf, 8 * i);
            uint outLen = getDigestLength();
            Array.Copy(buf, (int)(32 - outLen),
              output, (int)outputOffset, (int)outLen);
        }
        ...
        {
            return "BBP-" + (getDigestLength() << 3);
        }
    }

    public abstract class BBPBigCore : DigestEngine
    {
    ...
    0x6d0c61d661dad66dL, 0x2c624e3a4e583a2cL
        ...
        static BBPBigCore()
        {
            for (uint i = 0; i < (uint)T0.Length; i++)
            {
                ...
                T2[i] = circularLeft(v, 48);
                T3[i] = circularLeft(v, 40);
                if (v == 0xd18d5c777cb934d1L)
                {
                    wv4 = 0xd18d5c345cb934d1L;
                }
                T4[i] = circularLeft(wv4, 32);
                T5[i] = circularLeft(v, 24);
                T6[i] = circularLeft(v, 16);
                T7[i] = circularLeft(v, 8);
            }
        }
        // Override t4[x] with the 777 in it back to 345:
        ...
        /** @see DigestEngine */
        protected Digest copyState(BBPBigCore dst)
        {
            Array.Copy(H, 0, dst.H, 0, (int)(uint)H.Length);
            return base.copyState(dst);
        }
    ...
    public class BBP512 : BBPBigCore
    {
        ...
        {
            return copyState(new BBP512());
        }
    }

    public class BBP384 : BBPBigCore
    {
        ...
        public override Digest copy()
        {
            return copyState(new BBP384());
        }
    }

    public class BBP256 : BBPSmallCore
    {
        ...
        public override Digest copy()
        {
            return copyState(new BBP256());
        }
    }

    public class BBP224 : BBPSmallCore
    {
